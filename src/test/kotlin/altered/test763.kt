...
import org.example.altered.test763.RunChecker763.Companion.pool
...
class Processor {
    ...
    fun startProcessing() = runBlocking(pool) {
        launch(pool) { processOne() }
        launch(pool) { processTwo() }
        launch(pool) { sendData(1) }
        launch(pool) { sendData(2) }
        launch(pool) {
            for (i in 3..5) {
                sendData(i)
            }
