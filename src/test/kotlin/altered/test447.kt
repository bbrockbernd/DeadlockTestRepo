...
import org.example.altered.test447.RunChecker447.Companion.pool
...
class Processor {
    ...
    fun process() = runBlocking(pool) {
        coroutineScope {
            launch(pool) { firstAction() }
            launch(pool) { secondAction() }
            launch(pool) { thirdAction() }
            launch(pool) { fourthAction() }
        }
    }
