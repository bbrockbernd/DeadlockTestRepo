...
import org.example.altered.test115.RunChecker115.Companion.pool
...
fun produceData(dataSender: DataSender, data: Int) = runBlocking(pool) {
    launch(pool) {
        repeat(3) {
...
fun processData(dataReceiver: DataReceiver) = runBlocking(pool) {
    launch(pool) {
        repeat(3) {
