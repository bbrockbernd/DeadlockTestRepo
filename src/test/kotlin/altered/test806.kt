...
import org.example.altered.test806.RunChecker806.Companion.pool
...
fun function1(example: ChannelExample) = runBlocking(pool) {
    launch(pool) {
        example.ch1.send(1)
...
fun function2(example: ChannelExample) = runBlocking(pool) {
    launch(pool) {
...
fun function3(example: ChannelExample) = runBlocking(pool) {
    launch(pool) {
...
fun function4(example: ChannelExample) = runBlocking(pool) {
    launch(pool) {
...
fun main(): Unit= runBlocking(pool) {
    ...
    launch(pool) { function1(example) }
    launch(pool) { function2(example) }
    launch(pool) { function3(example) }
    launch(pool) { function4(example) }
