...
import org.example.altered.test82.RunChecker82.Companion.pool
...
fun mainFunction() = runBlocking(pool) {
    ...
    val job1 = launch(pool) { initiateSender(channels) }
    val job2 = launch(pool) { initiateReceiver(channels) }
    job1.join()
    job2.join()
}
