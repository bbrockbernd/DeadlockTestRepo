...
import org.example.altered.test993.RunChecker993.Companion.pool
...
fun firstFunction(ch1: Channel<Int>, ch2: Channel<Int>) = runBlocking(pool) {
    launch(pool) {
...
fun secondFunction(ch1: Channel<Int>, ch2: Channel<Int>) = runBlocking(pool) {
    launch(pool) {
        repeat(5) {
            ch1.send(it * 2)
        }
