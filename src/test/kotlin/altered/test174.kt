...
import org.example.altered.test174.RunChecker174.Companion.pool
...
fun function3(ch: Channel<Int>) = runBlocking(pool) {
    launch(pool) {
        val result = ch.receive()
...
fun function4(ch: Channel<Int>) = runBlocking(pool) {
    launch(pool) {
        ch.send(6)
    }
}
