...
import org.example.altered.test870.RunChecker870.Companion.pool
...
fun deadlockPart1(channel: Channel<Int>) = runBlocking(pool) {
    coroutineScope {
        launch(pool) {
            channel.send(2)
        }
...
fun deadlockPart2(channel: Channel<Int>) = runBlocking(pool) {
    coroutineScope {
        launch(pool) {
            channel.receive()
        }
