...
import org.example.altered.test306.RunChecker306.Companion.pool
...
fun startProcessing() {
    ...
    runBlocking(pool) {
        launch(pool) { producer.produce(listOf(1, 2, 3, 4)) }
        launch(pool) { consumer1.consume(channel2) }
        launch(pool) { consumer2.consume(channel3) }
