...
- mutexes 
*/
package org.example.altered.test818
import org.example.altered.test818.RunChecker818.Companion.pool
...
fun main(): Unit= runBlocking(pool) {
    ...
    launch(pool) { producer(channel) }
    launch(pool) { consumer(channel) }
    launch(pool) { multiplier(channel) }
    launch(pool) { printer(channel) }
}
