...
import org.example.altered.test136.RunChecker136.Companion.pool
...
fun function4(c: C) = runBlocking(pool) {
    launch(pool) {
        c.sendDouble(3.14)
    }
    launch(pool) {
...
fun function5(ch: Channel<Long>) = runBlocking(pool) {
    launch(pool) {
        ch.send(100L)
    }
    launch(pool) {
...
fun function8() = runBlocking(pool) {
    val ch = Channel<Char>()
    launch(pool) {
        ch.send('K')
    }
