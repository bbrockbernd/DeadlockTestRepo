...
- mutexes 
*/
package org.example.altered.test788
import org.example.altered.test788.RunChecker788.Companion.pool
...
fun main(): Unit= runBlocking(pool) {
    ...
    launch(pool) { coroutine1(channel1, channel2, channel3) }
    launch(pool) { coroutine2(channel2, channel3, channel4) }
    launch(pool) { coroutine3(channel1, channel5) }
    launch(pool) { coroutine4(channel4, channel5) }
    launch(pool) { coroutine5(channel3, channel5) }
...
class RunChecker788: RunCheckerBase() {
    companion object {
